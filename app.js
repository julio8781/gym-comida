...
function svgZanahoria({gordura=0.3, animo='normal', noche=false}={}){
  ...
  const cx = 150, topY = 102, botY = 300 + gordura*8;
  const half = w/2, bulge = gordura*46;
  ...
  const g1 = noche?'#2f8a42':'#6fb838', g2 = noche?'#37974a':'#8fce46', g3 = noche?'#2f8a42':'#7cc142';
  const flequillo = `
  ...
  const naranja = noche?'#e56a13':'#f7941d', borde = noche?'#c15409':'#d97614';
  const eyeY = topY + 30 + gordura*6, eyeDx = Math.max(11, w*0.14);
  const ex1 = cx-eyeDx, ex2 = cx+eyeDx;
  let ojos, boca;
  if(noche){
    ojos = `<path d="M ${ex1-9} ${eyeY} Q ${ex1} ${eyeY+8} ${ex1+9} ${eyeY}" stroke="#2a1a08" stroke-width="4" fill="none" stroke-linecap="round"/><path d="M ${ex2-9} ${eyeY} Q ${ex2} ${eyeY+8} ${ex2+9} ${eyeY}" stroke="#2a1a08" stroke-width="4" fill="none" stroke-linecap="round"/>`;
    boca = `<ellipse cx="${cx+8}" cy="${eyeY+30}" rx="${8+gordura*3}" ry="5" fill="#8a1f1f"/>`;
  } else if(animo==='mal'){
    ojos = `<path d="M ${ex1-9} ${eyeY-2} Q ${ex1} ${eyeY+6} ${ex1+9} ${eyeY-2}" stroke="#2a1a08" stroke-width="3.5" fill="none" stroke-linecap="round"/><path d="M ${ex2-9} ${eyeY-2} Q ${ex2} ${eyeY+6} ${ex2+9} ${eyeY-2}" stroke="#2a1a08" stroke-width="3.5" fill="none" stroke-linecap="round"/>`;
    boca = `<path d="M ${cx-18} ${eyeY+34} Q ${cx} ${eyeY+18} ${cx+18} ${eyeY+34}" stroke="#8a1f1f" stroke-width="4.5" fill="none" stroke-linecap="round"/>`;
  } else {
    ojos = `<ellipse cx="${ex1}" cy="${eyeY}" rx="12" ry="14" fill="#fff"/><ellipse cx="${ex2}" cy="${eyeY}" rx="12" ry="14" fill="#fff"/><circle class="pz-ojo" cx="${ex1+3}" cy="${eyeY+3}" r="6.5" fill="#2a1a08"/><circle cx="${ex1+5}" cy="${eyeY}" r="2.5" fill="#fff"/><circle class="pz-ojo" cx="${ex2+3}" cy="${eyeY+3}" r="6.5" fill="#2a1a08"/><circle cx="${ex2+5}" cy="${eyeY}" r="2.5" fill="#fff"/>`;
    if(animo==='feliz') boca = `<path d="M ${cx-18} ${eyeY+24} Q ${cx} ${eyeY+46} ${cx+20} ${eyeY+24} Q ${cx+12} ${eyeY+40} ${cx} ${eyeY+40} Q ${cx-10} ${eyeY+40} ${cx-18} ${eyeY+24} Z" fill="#8a1f1f"/><path d="M ${cx-15} ${eyeY+27} Q ${cx} ${eyeY+32} ${cx+17} ${eyeY+26} L ${cx+16} ${eyeY+29} Q ${cx} ${eyeY+35} ${cx-14} ${eyeY+30} Z" fill="#fff"/>`;
    else boca = `<path d="M ${cx-14} ${eyeY+26} Q ${cx} ${eyeY+34} ${cx+14} ${eyeY+26}" stroke="#8a1f1f" stroke-width="4" fill="none" stroke-linecap="round"/>`;
  }
  const gota = (animo==='mal' && !noche) ? `<path class="pz-gota" d="M ${cx+half*0.7} ${eyeY-4} q -6 12 0 18 q 6 -6 0 -18" fill="#7ec8ff"/>` : '';
  const zzz = noche ? `<text class="pz-z1" x="${cx+half*0.6+30}" y="${topY+48}" fill="#fff" font-size="22" opacity=".8">z</text><text class="pz-z2" x="${cx+half*0.6+46}" y="${topY+28}" fill="#fff" font-size="15" opacity=".6">z</text>` : '';
  const mano = (mx,my,fill)=>`<circle cx="${mx}" cy="${my}" r="8.5" fill="${fill}" stroke="${borde}" stroke-width="2"/><path d="M ${mx-6} ${my-6} l -3 -6 M ${mx-1} ${my-8} l -1 -7 M ${mx+4} ${my-8} l 2 -7 M ${mx+7} ${my-4} l 6 -4 M ${mx-8} ${my} l -7 -1" stroke="${fill}" stroke-width="4" fill="none" stroke-linecap="round"/>`;
  const pie = (px,py,dir)=>`<path d="M ${px} ${py} q ${dir*13} 2 ${dir*18} 9 q ${-dir*3} 5 ${-dir*10} 5 q ${-dir*10} 0 ${-dir*16} -6 Z" fill="${naranja}" stroke="${borde}" stroke-width="2"/><path d="M ${px+dir*7} ${py+3} l ${dir*3} 4 M ${px+dir*11} ${py+2} l ${dir*3} 5 M ${px+dir*15} ${py+1} l ${dir*2} 5" stroke="${borde}" stroke-width="1.5" fill="none" stroke-linecap="round"/>`;
  let brazos, piernas;
  const armY = topY + 56 + gordura*10;
  if(corre){
    brazos = `<path class="pz-bt" d="M ${cx-half*0.5} ${armY} Q ${cx-half-24} ${armY-10} ${cx-half-34} ${armY-30}" stroke="${borde}" stroke-width="10" fill="none" stroke-linecap="round"/><g class="pz-bt">${mano(cx-half-38, armY-34, naranja)}</g><path class="pz-bd" d="M ${cx+half*0.5} ${armY+8} Q ${cx+half+22} ${armY+6} ${cx+half+32} ${armY+24}" stroke="${naranja}" stroke-width="10" fill="none" stroke-linecap="round"/><g class="pz-bd">${mano(cx+half+36, armY+28, naranja)}</g>`;
    piernas = `<g class="pz-pt"><path d="M ${cx-14} ${botY-34} Q ${cx-34} ${botY-6} ${cx-50} ${botY+8}" stroke="${naranja}" stroke-width="12" fill="none" stroke-linecap="round"/>${pie(cx-50, botY+6, -1)}</g><g class="pz-pd"><path d="M ${cx+8} ${botY-18} Q ${cx+24} ${botY+8} ${cx+42} ${botY+18}" stroke="${naranja}" stroke-width="12" fill="none" stroke-linecap="round"/>${pie(cx+42, botY+16, 1)}</g>`;
  } else {
    const armDrop = animo==='mal' ? 26 : 34;
    // AHORA con clases animables en reposo: brazos se mecen
    brazos = `<g class="pz-mece-i"><path d="M ${cx-half*0.5} ${armY} Q ${cx-half-18} ${armY+armDrop-10} ${cx-half-24} ${armY+armDrop}" stroke="${borde}" stroke-width="10" fill="none" stroke-linecap="round"/>${mano(cx-half-26, armY+armDrop+4, naranja)}</g><g class="pz-mece-d"><path d="M ${cx+half*0.5} ${armY} Q ${cx+half+18} ${armY+armDrop-10} ${cx+half+24} ${armY+armDrop}" stroke="${naranja}" stroke-width="10" fill="none" stroke-linecap="round"/>${mano(cx+half+26, armY+armDrop+4, naranja)}</g>`;
    piernas = `<g><path d="M ${cx-12} ${botY-16} Q ${cx-16} ${botY+6} ${cx-28} ${botY+12}" stroke="${naranja}" stroke-width="12" fill="none" stroke-linecap="round"/>${pie(cx-28, botY+10, -1)}</g><g><path d="M ${cx+12} ${botY-16} Q ${cx+16} ${botY+6} ${cx+28} ${botY+12}" stroke="${naranja}" stroke-width="12" fill="none" stroke-linecap="round"/>${pie(cx+28, botY+10, 1)}</g>`;
  }
  const claseAnim = corre ? 'pz-corre' : (gordura>0.6 ? 'pz-resp pz-lento' : 'pz-resp');
  const armOx = cx-half*0.5, armOx2 = cx+half*0.5;
  return `<svg viewBox="0 0 300 380" width="150" xmlns="http://www.w3.org/2000/svg" style="--armYi:${armY}px;--armXi:${armOx}px;--armXd:${armOx2}px">
    <g class="${claseAnim}">
      ${flequillo}${brazos}${piernas}
      <path d="${body}" fill="${naranja}" stroke="${borde}" stroke-width="3.5"/>
      <path d="M ${cx-half*0.4} ${eyeY+2} Q ${cx} ${eyeY-4} ${cx+half*0.4} ${eyeY+2}" stroke="#ffb85c" stroke-width="3" fill="none" opacity=".5"/>
      ${ojos}${boca}${gota}${zzz}
    </g>
  </svg>`;
}
